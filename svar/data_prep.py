...
TARGET_MONTHS = 120  # aim for 10 years — captures full available history (binding: Dec 2017)
...
def _load_extended() -> pd.DataFrame | None:
    ...
    try:
        ...
        from external.reserves    import fetch_exchange_rate
        ...
        return _assemble(cpi_df, fx_df, cons_df, emae_df)
...
def _assemble(cpi_df: pd.DataFrame, fx_df: pd.DataFrame,
              cons_df: pd.DataFrame, emae_df: pd.DataFrame) -> pd.DataFrame:
    """Merge four DataFrames onto a common monthly date index."""
    # CPI mom
